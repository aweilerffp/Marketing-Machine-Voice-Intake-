...
function parseArgs() {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === '--section' || args[i] === '-s') {
      section = (args[i + 1] || '').toLowerCase();
      i++;
    } else if (!args[i].startsWith('-')) {
      transcriptArg = args[i];
    }
  }

  if (!SECTIONS[section]) {
    console.error(`Invalid section: "${section}". Must be a, b, or c.`);
    console.error('  a = Brand Voice (questions 1-22)');
    console.error('  b = ICP (questions 23-33)');
    console.error('  c = Channels (questions 34-41)');
    process.exit(1);
  }

  return { section, transcriptArg };
}
...
async function resolveTranscriptPath(arg) {
  ...
  if (!transcript) {
    throw new Error(
      ...
      'Usage: node --env-file=.env src/generate.mjs --section a <filename>\n' +
...
async function main() {
  const { section, transcriptArg } = parseArgs();
  const config = SECTIONS[section];

  console.log(`Section: ${config.name} (${section.toUpperCase()})`);
  ...
  const [transcript, promptTemplate, questions, template, schemaRaw] = await Promise.all([
    ...
    readProjectFile(config.prompt, 'generation prompt'),
    readProjectFile(config.questions, 'discovery questions'),
    readProjectFile(config.template, 'deliverable template'),
    readProjectFile(config.schema, 'output schema'),
  ]);
  ...
  const outputPath = join(projectRoot, 'outputs', `${config.outputPrefix}-${inputName}-${timestamp}.json`);
  ...
  console.log('\n--- Theme Summary ---');
  for (const [key, themeSection] of Object.entries(result.themes)) {
    const insights = themeSection.insights?.length || 0;
    const quotes = themeSection.verbatim_quotes?.length || 0;
    const donts = themeSection.explicit_dont?.length || 0;
    console.log(`  ${themeSection.section_title}: ${insights} insights, ${quotes} quotes, ${donts} don'ts`);
  }
